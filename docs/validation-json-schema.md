...
| `keyword` | `string \| null` | JSON Schema keyword that failed (`type`, `required`, `enum`, …). Set on schema violations, and additionally `"required"` when a required parameter / header is missing; `null` otherwise. |
