...
- **`allOf` is unioned; `anyOf` / `oneOf` are skipped.** `allOf` semantics are AND, so the union of `required` arrays across branches is sound. `anyOf` / `oneOf` are disjunctions and there is no safe AND-semantic for "required" across them; the asserter conservatively treats those schemas as having no declared `required`. If your spec relies on composition for required-set assembly, prefer `allOf` or open a follow-up issue with your use case.
