...
- **Map-shaped objects are excluded from drift reporting, and their `additionalProperties` schemas are not walked.** When a schema node declares `additionalProperties` (schema form or boolean `true`) and no `properties`, the author has said "this object is a map": its observed keys are data, not shape, so they are **not** reported as always-present fields missing from `required` (issue #437) — adding dynamic keys to `required` would be factually wrong. The asserter also does not descend into the `additionalProperties` schema to look for `required` on the map's values; walk-depth follows declared `properties` and `items` only. **Consequence:** dynamically-keyed response objects (`{"user-42": {...}, "user-43": {...}}`-style maps) silently miss strict-required coverage on both their keys and their values. If you need strict-required coverage on map-shaped responses, pin the shape with declared `properties` instead. A node that declares at least one `properties` entry *alongside* `additionalProperties` keeps ordinary drift reporting — a partially-fixed shape still makes "this key is always present" a meaningful claim.
...
- **Empty `{}` and `[]` at child positions are skipped.** `StrictRequiredBodyWalker` normalizes any `stdClass` node to a PHP array on entry before checking for emptiness, so an empty object and an empty list both arrive at the same ambiguous `[]` check regardless of whether the caller decoded JSON objects as arrays or as `stdClass`. This keeps observations consistent across the framework adapters, legacy assoc-array callers of the public validators, and mixed-version sidecar merges — none of which can be assumed to agree on how an empty object was decoded. To avoid polluting observations with ambiguous shapes, the walker records empty `{}` only at the response root (preserving "empty response collapses the intersection"). Empty arrays at nested positions contribute no pointer to that observation — combined with the tracker's "absence drops the pointer" rule, a sometimes-empty nested array silently drops its `[*]` row from cross-response analysis. If you need strict-required coverage on a nullable array property, pin the shape with a more specific spec (e.g. `oneOf` with concrete element types — though see disjunction limitation above).
