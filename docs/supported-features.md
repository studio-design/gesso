...
- **Multipart file parts**: a file part's bytes are never read. Its presence satisfies `required`, its client filename stands in for the `type: string, format: binary` value (so a `minLength` / `pattern` on a binary property is measured against the filename, not the file), and its declared `Content-Type` is checked against the `encoding` object.
- **Multipart `encoding` object**: `encoding.<part>.contentType` is enforced — a part whose Content-Type is outside the declared media type, range, or comma-separated list fails, and a part declared as JSON is decoded before its subschema is applied. `encoding.<part>.headers` / `style` / `explode` are not consulted.
