...
- **Multipart `encoding` object**: `encoding.<part>.contentType` is enforced — a part whose Content-Type is outside the declared media type, range, or comma-separated list fails. Per RFC 7578 §4.4 a part with no Content-Type of its own is treated as `text/plain` rather than waved through. A part that may be JSON is decoded before its subschema is applied; when `contentType` is omitted the OAS 3.0.3 per-property default decides (`application/json` for an object, the inner type's default for an array, `application/octet-stream` for a binary string, `text/plain` otherwise), so an object part is JSON without an `encoding` entry. A string that does not parse fails only when JSON is the part's sole candidate; with another media type declared alongside it, the value is left for the subschema to judge. `encoding.<part>.headers` / `style` / `explode` are not consulted.
