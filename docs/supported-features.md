...
- **Multipart file parts**: a file part's bytes are never read. Its presence satisfies `required`, its client filename stands in for the `type: string, format: binary` value (so a `minLength` / `pattern` on a binary property is measured against the filename, not the file), and its declared `Content-Type` is checked against the `encoding` object. A file whose upload failed (anything other than `UPLOAD_ERR_OK` — no file sent, size limit, partial write) is dropped before validation, so it cannot satisfy a `required` part.
- **Multipart `encoding` object**: `encoding.<part>.contentType` is enforced — a part whose Content-Type is outside the declared media type, range, or comma-separated list fails. Per RFC 7578 §4.4 a part with no Content-Type of its own is treated as `text/plain` rather than waved through. Non-file parts are the limit of this: no adapter preserves a plain field's part headers, so their declared `contentType` cannot be matched — what is enforced instead is that a property declaring raw bytes (`format: binary`, or a 3.1+ `contentMediaType` with no `contentEncoding`) actually arrives as a file part. A part that may be JSON is decoded before its subschema is applied; when `contentType` is omitted the OAS 3.0.3 per-property default decides (`application/json` for an object, the inner type's default for an array, `application/octet-stream` for a binary string, `text/plain` otherwise), so an object part is JSON without an `encoding` entry. When several media types are declared, OAS 3.2 §4.15.4.1 selects between them by the part's own Content-Type and rules out sniffing — since that header is not observable here, a mixed list is decoded as JSON only when the property schema could not hold a plain string anyway (`type: object` / `type: array`); otherwise the raw value goes to the subschema unchanged. Content that does not parse is an error only in those two cases, where JSON was the sole readable choice. `encoding.<part>.headers` / `style` / `explode` are not consulted.
